...
double simulator::eventRate_infectiousProgress(unsigned int i){
	...
	double I = census_status(_nE+1+i);
	
	double KI = 0.0;
	bool found = false;
	
	if (_Kfct == "one")   { KI = 1.0 ; found = true;}
	if (_Kfct == "lin")   { KI = I/_popSize ; found = true;}
	if (_Kfct == "sqrt")  { KI = sqrt(I/_popSize) ; found = true;}
	if (_Kfct == "exp")   { KI = exp(-_Kfct_prm[0] * I/_popSize) ; found = true;}
...
vector< vector<double> > simulator::get_GIbck(){
	...
	for (int i=0; i<_popSize;  i++) {
		t.push_back(_indiv[i].get_timeDiseaseAcquisition());
		gibck.push_back(_indiv[i].get_GIbck());
	}
